...
def _initialize_terminal_worker(solver):
    global _terminal_worker_solver
    _terminal_worker_solver = solver


def _solve_sampled_terminal_candidate(
    candidate_data, sample_index, t, x0, forced_alpha, u1_0, u2_0
):
    """Process-pool entry point for one discrete terminal-state solve."""
    solver = copy.copy(_terminal_worker_solver)
    solver.Solution = copy.deepcopy(_terminal_worker_solver.Solution)
    solver.Solver = None
    try:
        solver._step_once(
            t,
...
class DGSolver:
    ...
    def _step_over_sampled_terminal_states(
        ...
        candidate_indices = np.where(
            ...
            & (sample_times <= t + (1.25 * self.N) * self.dt)
        )[0]
